...
def test_installation():
    ...
    # Test 6: Check EveLogLite integration files
    eveloglite_dir = os.path.join(os.path.dirname(script_dir), "EveLogLite-master")
    if os.path.exists(eveloglite_dir):
        print("[OK] EveLogLite integration available")
    else:
        print("[INFO] EveLogLite files not found (optional integration)")
        
    # Test 7: Check GUI components
    gui_script = os.path.join(script_dir, "crash_monitor_gui.py")
    if os.path.exists(gui_script):
        print("[OK] GUI interface available")
    else:
        print("[INFO] GUI interface not found")
    
    print()
    print("Installation test completed successfully!")
    print("You can now run the monitor with:")
    print("   scripts\\run_gui.bat                    (GUI interface)")
    print("   scripts\\run_monitor.bat                (Command line)")
    print("   python\\enhanced_crash_monitor.py       (EveLogLite integration)")
    print()
